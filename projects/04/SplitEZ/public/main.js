document.addEventListener('DOMContentLoaded', () => {
    ...
    // Event listeners cho input số tiền để format theo định dạng VNĐ
    amountInput.addEventListener('input', formatInputCurrency);
    amountInput.addEventListener('blur', formatInputCurrency);
    amountInput.addEventListener('focus', function() {
        // Khi focus vào input, chuyển về định dạng số để dễ nhập
    ...
    /**
     * Format số tiền trong input khi người dùng đang nhập
     */
    function formatInputCurrency() {
        // Loại bỏ tất cả các ký tự không phải số
        let value = this.value.replace(/[^\d]/g, '');
        
        // Nếu có giá trị thì định dạng nó
        if (value) {
    ...
    function addExpense() {
        ...
        const amountValue = amountFormatted.replace(/[^\d]/g, '');
        ...
        // Hiển thị thông báo thành công
        showNotification(`Đã thêm khoản chi phí ${formatCurrency(amountValue)} cho ${payer}`, 'success');
    }
